...
public abstract class BaseServiceImpl<T extends BaseEntity> implements BaseService<T>{
	...
	public PageView<T> queryPageData(Long currentPageNum, Long pageSize, String wherSQL, List<Object> whereParams,
	 ...
	 @Override
	 public List<T> queryData(String wherSQL,List<Object> whereParams,LinkedList<String> groupBy) {
		// 第一步、获取要查询的字段
		String queryFields = getFields();
		if (StringUtils.isBlank(queryFields)) {
			throw new BizException(StatusCode.QUERY_FIELDS_NOT_FOUND);
		...
		String where = StringUtils.isBlank(wherSQL) ? "" : " where " + wherSQL;
		// 第四步、获取groupBy条件
		String groupBySql = this.buildGroupBy(groupBy);
		// 第五步、拼接sql语句
		String selectSQL = " SELECT " + queryFields + " from " + tableName + where  + groupBySql;
		// 第六步、设置参数
	 ...
	 public List<T> queryData() {
		 return this.queryData(null,null,null);
	 }
	 
	 
	 /**
	 * @Description: 组装group by 语句
	 * @author: 朱俊亮
	 * @date: 2018年10月17日 上午9:03:51
	 * @param: groupBy 传入一个含有分组字段的参数
	 * @return: String 返回组装好的group by条件
	 */
	protected String buildGroupBy(LinkedList<String> groupBy){
		 StringBuffer groupBySql = new StringBuffer("");
		 if (groupBy != null && !groupBy.isEmpty()) {
			 groupBySql.append(" group by ");
				for (String key : groupBy) {
					if(!StringUtils.isBlank(key)){
						groupBySql.append(key).append(" ").append(",");
					}
				}
